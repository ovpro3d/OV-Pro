...
class PseudoProcessor(object):
    ...
    def relabel_gt_boxes(self, gt_boxes: Tensor) -> Tensor:
        ...
        for b in range(gt_boxes.shape[0]):
            for i in range(gt_boxes.shape[1]):
                curr_label = int(gt_boxes[b, i, -1].item()) # 获取GT标签的class
                if curr_label in self.gt_known_to_full_labels: # known_class_name: 'car', 'construction_vehicle', 'trailer', 'barrier', 'bicycle', 'pedestrian' 六个；
                    # only modify if not the non-match id      #对这六个标签进行重新映射：{1: 1, 2: 3, 3: 5, 4: 6, 5: 8, 6: 9},其余四个类别为unknown class_0,4,7,10
    ...
    def combine_gt_with_pseudos(self, gt_boxes: Tensor, pseudo_boxes: Tensor) -> Tensor:
        ...
        MN = 0
        MN_ = gt_boxes.shape[1] + pseudo_boxes.shape[1] # adds redundant padding #[B,66,10] [B,12,8]
        ...
        for b in range(gt_boxes.shape[0]):
            ...
            num_gt = cur_gt_boxes.shape[0] #[66]->[31] GT里面为什么存在w，h<0的框
            num_pseudo = cur_pseudo_boxes.shape[0] #[12]

            # STATS LOGGING ############################
            curr_stats['num_gt'] += num_gt
            curr_stats['num_pseudo'] += num_pseudo

            curr_stats = count_classes(cur_gt_boxes, curr_stats, self.all_class_names) # 计算得到GT框中的每个类别
            ...
            ret_boxes[b, :num_gt] = cur_gt_boxes #在初始化的全零矩阵中添加可用的GT_box
            pseudo_dim = cur_pseudo_boxes.shape[-1] # 8
            ret_boxes[b, num_gt:num, :(pseudo_dim - 1)] = cur_pseudo_boxes[..., :-1] #初始矩阵中添加pseudo_box 且舍去最后一个class维度
            ret_boxes[b, num_gt:num, -1] = cur_pseudo_boxes[..., -1] # 加上最后一个维度
    ...
    def __call__(self, batch_dict):
        ...
        gt_boxes = self.combine_gt_with_pseudos(gt_boxes, pseudo_boxes) # [8,68,10]
        ...
        batch_dict['gt_boxes'] = gt_boxes # 对gt_box进行了修改，加入了pseudo_boxes
