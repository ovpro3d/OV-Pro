...
class C3PG(nn.Module):
    def __init__(
        ...
        self.ENABLE_C3PG = model_cfg is not None and model_cfg.get('ENABLE_C3PG', True)
        self.c3pg_delta = 0.5   # Balance weight for appearance vs geometry
        self.c3pg_lambda = 0.6  # Threshold for merging
        self.c3pg_psi = 330.21  # Max lateral overlap width
        # if self.SAVE_BLEND_FILES:
            # score_thr = 0.3
        if self.ENABLE_C3PG:
            print("Loading DINOv2 for Online Embedding Extraction...")
        ...
        preds_path = model_cfg.get('PREDS_PATH', '/OV-Pro/uqdetche/GLIP/jsons/OWL_')
    ...
    def calc_inlier_scores(self, anchor: Tensor, box_proposals: Tensor, points: Tensor, dirs: Tensor, mags: Tensor):
        ...
        return scores
    def extract_online_embeddings(self, batch_dict, det_boxes, det_batch_idx, det_cam_idx):
        if not self.ENABLE_C3PG or len(det_boxes) == 0:
            return None
        ...
        for i in range(len(det_boxes)):
            b_idx = det_batch_idx[i]
            c_idx = det_cam_idx[i]
            box = det_boxes[i] 
            img = images[b_idx, c_idx]
            ...
            if x2 <= x1 or y2 <= y1:
                ...
                crop = torch.zeros((3, 224, 224), device=img.device)
            else:
                
                crop = img[:, y1:y2, x1:x2]
                crop = self.dino_preprocess(crop)
        ...
        with torch.no_grad():
            for i in range(0, len(crops_tensor), chunk_size):
                batch_crops = crops_tensor[i : i + chunk_size]
                feats = self.dinov2_model(batch_crops)
                embeddings_list.append(feats)

        embeddings = torch.cat(embeddings_list, dim=0)
        return embeddings
    def correct_cross_view_instances(self, det_boxes, det_labels, det_scores, det_batch_idx, det_cam_idx, embeddings=None):
        """
        Implementation of Section 3.2: Cross-View instance consistency constrained prototype generation (C3PG).
        Algorithm 1: Coordinate System Transformation & Correction.
        """
        if not self.ENABLE_C3PG:
            return det_boxes, det_labels, det_scores, det_cam_idx
        panoramic_order = [3, 4, 1, 0, 2, 5] 
        
        img_width = self.image_size[1] # 1600
        keep_mask = torch.ones(det_boxes.shape[0], dtype=torch.bool, device=det_boxes.device)
        if self.box_fmt == 'xyxy':
        ...
        if heights.numel() > 1:
            delta_h_max = torch.abs(heights.max() - heights.min()).clamp(min=10.0) 
        else:
            delta_h_max = 200.0 

        num_cams = len(panoramic_order)
        for k in range(num_cams):
            cam_i_idx = panoramic_order[k]
            cam_j_idx = panoramic_order[(k + 1) % num_cams] # Wrap around for full circle
            
            mask_i = (det_cam_idx == cam_i_idx) & keep_mask
            mask_j = (det_cam_idx == cam_j_idx) & keep_mask
            
            idxs_i = torch.nonzero(mask_i).flatten()
            idxs_j = torch.nonzero(mask_j).flatten()
            
            if len(idxs_i) == 0 or len(idxs_j) == 0:
                continue
            
            # ---------------------------------------------------------------------
            # 3. Pairwise Similarity Check
            # ---------------------------------------------------------------------
            for idx_i in idxs_i:
              
                box_i = det_boxes[idx_i]
                if self.box_fmt == 'xyxy':
                    ui_max = box_i[2]
                    vi_min, vi_max = box_i[1], box_i[3]
                    hi = box_i[3] - box_i[1]
                else:
                    ui_max = box_i[0] + box_i[2]
                    vi_min, vi_max = box_i[1], box_i[1] + box_i[3]
                    hi = box_i[3]

                for idx_j in idxs_j:
                    if det_batch_idx[idx_i] != det_batch_idx[idx_j]:
                        continue
                    if det_labels[idx_i] != det_labels[idx_j]:
                        continue

                    # Get Box J properties
                    box_j = det_boxes[idx_j]
                    if self.box_fmt == 'xyxy':
                        uj_min = box_j[0]
                        uj_max = box_j[2]
                        vj_min, vj_max = box_j[1], box_j[3]
                        hj = box_j[3] - box_j[1]
                    else:
                        uj_min = box_j[0]
                        uj_max = box_j[0] + box_j[2]
                        vj_min, vj_max = box_j[1], box_j[1] + box_j[3]
                        hj = box_j[3]

                    # -------------------------------------------------------------
                    # 4. Geometric Similarity (Eq 2)
                    # -------------------------------------------------------------
                    gap_dist = (img_width - ui_max) + uj_min
                    
                    if abs(gap_dist) > self.c3pg_psi:
                        continue
                        
                    s_geo = torch.exp(-torch.abs(hi - hj) / delta_h_max)

                    # -------------------------------------------------------------
                    # 5. Appearance Similarity (Eq 3)
                    # -------------------------------------------------------------
                    if embeddings is not None:
                        ai = embeddings[idx_i]
                        aj = embeddings[idx_j]
                        sim_cos = F.cosine_similarity(ai.unsqueeze(0), aj.unsqueeze(0))
                        s_app = (sim_cos + 1) / 2.0
                    else:
                        s_app = 1.0 # Default if no embeddings
                    
                    # -------------------------------------------------------------
                    # 6. Total Similarity & Merging (Eq 4)
                    # -------------------------------------------------------------
                    s_total = self.c3pg_delta * s_geo + (1 - self.c3pg_delta) * s_app
                    
                    if s_total >= self.c3pg_lambda:
                        # Merge Step: Correct Box I, Remove Box J
                        # o'_i = (ui_min, uj_max_global, min_v, max_v)
                        # To project uj_max to Box I's frame extended, we add img_width.
                        # Note: We use box_j[2] (x2) directly, preserving the offset.
                        
                        new_x2 = img_width + uj_max
                        
                        # Update Box I in place
                        if self.box_fmt == 'xyxy':
                            det_boxes[idx_i, 2] = new_x2 
                            det_boxes[idx_i, 1] = min(vi_min, vj_min)
                            det_boxes[idx_i, 3] = max(vi_max, vj_max)
                        else:
                            # Update width and height
                            current_x1 = det_boxes[idx_i, 0]
                            det_boxes[idx_i, 2] = new_x2 - current_x1 # New width
                            new_y1 = min(vi_min, vj_min)
                            det_boxes[idx_i, 1] = new_y1
                            det_boxes[idx_i, 3] = max(vi_max, vj_max) - new_y1 # New height
                        
                        # Suppress Box J
                        keep_mask[idx_j] = False

        return det_boxes[keep_mask], det_labels[keep_mask], det_scores[keep_mask], det_cam_idx[keep_mask]
    def get_proposals(self, batch_dict):
        ...
        det_embeddings = batch_dict.get('det_embeddings', None)
        det_boxes, det_labels, det_scores, det_batch_idx, det_cam_idx = self.image_detector(batch_dict)
        if self.ENABLE_C3PG:
            det_embeddings = self.extract_online_embeddings(
                batch_dict, det_boxes, det_batch_idx, det_cam_idx
            )
        det_boxes, det_labels, det_scores, det_cam_idx = self.correct_cross_view_instances(
            det_boxes, det_labels, det_scores, det_batch_idx, det_cam_idx, embeddings=det_embeddings
        )
        ...
        for b in range(batch_size):
            ...
            for c in self.image_order:
                ...
                cam_points = cam_points[cam_mask] # [3407, 3]
            ...
            for frust_idx, (frust_box, c, box, box_points, weighted_centre_xyz, label, score) in \
                ...
                curr_box_proposals = self.base_boxes[label.item() - 1].clone().unsqueeze(0).repeat(bev_pts_xyz.shape[0], 1, 1)  
                ...
                closest_ranking = torch.softmax(- curr_corner_proposals.clone().norm(dim=2), dim=1) 
