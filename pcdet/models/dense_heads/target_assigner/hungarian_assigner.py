...
def overlaps(boxes1, boxes2):
    ...
    # 3d overlaps
    overlaps_3d = overlaps_bev.to(boxes1.device) * overlaps_h #BEV重叠度和高度重叠度相乘 [200,22]

    volume1 = (boxes1[:, 3] * boxes1[:, 4] * boxes1[:, 5]).view(-1, 1) # [200,1]
    volume2 = (boxes2[:, 3] * boxes2[:, 4] * boxes2[:, 5]).view(1, -1) # [1,22]
        
    iou3d = overlaps_3d / torch.clamp(volume1 + volume2 - overlaps_3d, min=1e-8) #volume1 + volume2 - overlaps_3d：这部分计算了两个体积的和，减去它们的交集，得到并集的体积。简单来说，这是两个边界框合并后的总体积。
...
class HungarianAssigner3D:
    ...
    def bevbox_cost(self, bboxes, gt_bboxes, point_cloud_range):
        ...
        reg_cost = torch.cdist(normalized_bboxes_xy, normalized_gt_bboxes_xy, p=1) #使用L1范式计算位置cost
    ...
    def assign(self, bboxes, gt_bboxes, gt_labels, cls_pred, point_cloud_range):
        ...
        assigned_gt_inds = bboxes.new_full((num_bboxes,), -1, dtype=torch.long) # 全部值初始为-1，形状为num_bboxes[200]
        assigned_labels = bboxes.new_full((num_bboxes,), -1, dtype=torch.long) # 全部值初始为-1，形状为num_bboxes[200]
        ...
        cls_cost = self.focal_loss_cost(cls_pred[0].T, gt_labels) # 计算类别cost [200, 22]
        reg_cost = self.bevbox_cost(bboxes, gt_bboxes, point_cloud_range) # 计算box_cost [200, 22])
        iou_cost, iou = self.iou3d_cost(bboxes, gt_bboxes) # 计算iou_cost [200, 22])
        ...
        matched_row_inds = torch.from_numpy(matched_row_inds).to(bboxes.device) #[22] pred
        matched_col_inds = torch.from_numpy(matched_col_inds).to(bboxes.device) #[22] gt
        ...
        assigned_labels[matched_row_inds] = gt_labels[matched_col_inds] #得到匹配的gt_label

        max_overlaps = torch.zeros_like(iou.max(1).values) # [200]
        max_overlaps[matched_row_inds] = iou[matched_row_inds, matched_col_inds] #得到匹配的IoU值 [200]
