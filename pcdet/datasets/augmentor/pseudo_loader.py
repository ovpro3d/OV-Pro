...
def remove_empty(pseudo_boxes: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    """
    non_empty = np.bitwise_and(pseudo_boxes[:, 3] > 0, pseudo_boxes[:, 4] > 0) #用于执行按位与操作。在处理图像或者二维数组时，np.bitwise_and可以用来提取两个数组中对应位置都为True（或非零）的元素。
...
class PseudoSampler(object):
    ...
    def __init__(self, class_labels: list, known_class_labels: list, unknown_class_labels: list, 
        ...
        self.max_queue_size_per_class = max_queue_size_per_class # 100
...
class PseudoLoader(object):
    ...
    def __init__(self, known_class_names: list, pseudo_path='pseudo_labels/frustum_proposals/', self_train_path: str = None, 
        ...
        self.ego_vehicle = torch.tensor([[0, -1.0, (-5.0 + 3.0)/2.0, 5.0, 3.0, 8.0, np.pi/2.0]], dtype=torch.float32, device='cpu') #[1, 7]
    ...
    def load_pseudos(self, batch_dict, unknowns_only=True, folder: str = None, 
        ...
        try:
            preds_dicts = torch.load(pseudo_path, map_location='cpu') # 读取pseudo_label
        ...
        if unknowns_only: # don't need pseudo labels for knowns 过滤掉known类别
            pred_mask = np.zeros_like(pred_labels, dtype=bool)
            for i, lbl in enumerate(pred_labels):
                # pred mask
                pred_mask[i] = lbl in self.unknown_class_labels #类别2,4,7,10为unknown classes
        ...
        if pred_boxes.shape[0] == 0:
            return np.zeros((0, 8)), np.zeros((0)) #如果不存在unknown_class则返回一个空数组
        ...
        pseudo_boxes[:pred_boxes.shape[0], :box_dim] = pred_boxes[:, :box_dim] # 将框的属性放入初始化好的pseudo_boxes中
        pseudo_boxes[:pred_boxes.shape[0], -1] = pred_labels # 将框的类别放入初始化好的pseudo_boxes中
    ...
    def load_frustum_pseudos(self, batch_dict: Dict) -> Dict:
        # frustum pseudos (no filtering, score is based on image detector) 视锥中的pseudo_lable(不滤波，评分基于图像检测器)
        pseudo_boxes, pseudo_scores = self.load_pseudos(batch_dict, filter_by_score=False) # no filtering for frustum pseudos 提取出unknown_class
    ...
    def load_selftrain_pseudos(self, batch_dict: Dict) -> Dict:
        ...
        # do nms with self-training and frustum pseudos 进行了过滤 self-training的pseudo_label为空时为什么还进行顾虑
        ...
        # add ego to gt to remove any pseudos that overlap 将ego状态也加入到gt中，以移除任何重叠的pseudo_lable
        gt_boxes_tensor = torch.cat((gt_boxes_tensor[:, :7], self.ego_vehicle.to(gt_boxes_tensor.device)), dim=0)
        ...
        if gt_boxes_tensor.numel() > 0 and pseudo_boxes_tensor.numel() > 0:
            ...
            pseudo_boxes = pseudo_boxes_tensor[ious_mask].numpy() # 去除与gt的overlap大于0.1的pseudo_label
        ...
        # remove boxes that are empty 去除框的wh小于0的pseudo_label
        ...
        if self.copy_st_only:
            ...
            self.copy_boxes = pseudo_boxes[st_mask].copy() # [3,8]->[0]
